...
show_help() {
    ...
    echo "  bash scripts/third_party_builders/builder_spdlog.sh --platform x86_64"
    echo ""
    echo "  # 从任何位置运行"
    echo "  bash /path/to/project/scripts/third_party_builders/builder_spdlog.sh \\"
...
echo "[spdlog构建器] 平台: $PLATFORM"
echo "[spdlog构建器] 交叉编译前缀: $CROSS_COMPILE_PREFIX"
...
# 克隆或更新代码
if [ ! -d "spdlog" ]; then
    echo "[spdlog构建器] 克隆spdlog代码..."
    git clone https://gitee.com/mirror-luyi/spdlog.git
    if [ $? -ne 0 ]; then
        echo "[spdlog构建器] 错误: 克隆spdlog代码失败"
        exit 1
    fi
    
    # 切换到指定版本
    cd spdlog
    echo "[spdlog构建器] 切换到版本 v1.14.1..."
    git checkout v1.14.1
    if [ $? -ne 0 ]; then
        echo "[spdlog构建器] 错误: 切换到版本 v1.14.1 失败"
        exit 1
    fi
else
    echo "[spdlog构建器] spdlog目录已存在，跳过git clone步骤"
    cd spdlog
    
    # 如果目录已存在，也尝试切换到指定版本以确保版本一致
    echo "[spdlog构建器] 确保使用版本 v1.14.1..."
    CURRENT_BRANCH=$(git describe --tags 2>/dev/null || echo "unknown")
    echo "[spdlog构建器] 当前版本: $CURRENT_BRANCH"
    
    if [ "$CURRENT_BRANCH" != "v1.14.1" ]; then
        echo "[spdlog构建器] 切换到版本 v1.14.1..."
        git checkout v1.14.1
        if [ $? -ne 0 ]; then
            echo "[spdlog构建器] 警告: 切换到版本 v1.14.1 失败，使用当前版本"
            echo "[spdlog构建器]       如果需要特定版本，请手动删除目录并重新运行"
        fi
    fi
fi
...
if [ $? -ne 0 ]; then
    echo "[spdlog构建器] 错误: spdlog配置失败"
    exit 1
fi

echo "[spdlog构建器] 编译spdlog..."
CPU_CORES=$(nproc 2>/dev/null || echo 4)
echo "[spdlog构建器] 使用 $CPU_CORES 个CPU核心进行编译"
make -j$CPU_CORES

if [ $? -ne 0 ]; then
    echo "[spdlog构建器] 错误: spdlog编译失败"
    exit 1
fi

echo "[spdlog构建器] 安装spdlog..."
make install

if [ $? -ne 0 ]; then
    echo "[spdlog构建器] 错误: spdlog安装失败"
    exit 1
fi
